...
contract("'EvenVoterRewardModel' contract test.", async accounts => {
  ...
  it("Can follow corretly the 1st scenario of which contest defined with 'EvenVoterRewardModel'.", async() => {
    
    const [chance, admin] = await prepareFixtures();
    const rwdMdl = await prjMgrContr.getRewardModel(1);
    ...
    const rwdsVrfyFunc = function(rwds){
       ...
       *   voter.0 : 10 ESV   voter.1 : 10 ESV   voter.2 : 10 ESV   
       */
      assert.equal(rwds.find(e => e.addr == votees[0].addr).esv, 42);
      assert.equal(rwds.find(e => e.addr == votees[1].addr).esv, 28);
      assert.equal(rwds.find(e => e.addr == voters[0].addr).esv, 10);
      assert.equal(rwds.find(e => e.addr == voters[1].addr).esv, 10);
      assert.equal(rwds.find(e => e.addr == voters[2].addr).esv, 10);      
      // verify total reward
    ...
    await verifyScenario(
        prj, vteeAddrs, vts, "Scenario 1 (Contest Scenario) with 'EvenVoterRewardModel'", rwdsVrfyFunc);
    
  });
  
  it("Can follow corretly the 2nd scenario with 'EvenVoterRewardModel'.", async() => {
    ...
    await verifyScenario(prj, vteeAddrs, vts, "Scenario 2 with 'EvenVoterRewardModel'", rwdsVrfyFunc); 
  });
  
  it("Can vote according to scenario 3 with 'EvenVoterRewardModel'", async() => {
    ...
    await verifyScenario(prj, vteeAddrs, vts, "Scenario 3 with 'EvenVoterRewardModel'", rwdsVrfyFunc); 
     
  });  
  
  it("Can vote according to scenario 4 with 'EvenVoterRewardModel'", async() => {
    ...
    await verifyScenario(prj, vteeAddrs, vts, "Scenario 4 with 'EvenVoterRewardModel' ", rwdsVrfyFunc); 
